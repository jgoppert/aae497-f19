...
public:
    ...
    void insert(const Landmark &lm, int depth = 0)
    {
        ...
        {
            if (lm.pos.x > m_center.x) //insert at east
            {
                if (lm.pos.y > m_center.y) //insert at north
                {
                    Position center{m_center.x + m_size / 2, m_center.y + m_size / 2};
                    subInsert(m_NE, lm, center);
                }
                else //insert at south
                {
                    Position center{m_center.x + m_size / 2, m_center.y - m_size / 2};
    ...
    std::list<Landmark> search(const Position &position, double radius)
    {
        ...
        // if(covers(position,radius))
        // {
        //     std::cout<<" |covers: "<< m_center.x<<" ,"<<m_center.y<<" |size: "<<m_size<<std::endl;
        //     std::cout<<"Has: "<<m_landmarks.size()<<std::endl;
        ...
        if (m_size < m_resolution) 
        {
            close_landmarks.splice(close_landmarks.end(), std::list<Landmark>(m_landmarks)); 
            return close_landmarks;
        }
        if(overlapses(position,radius))
        {
            if(m_NE.get())
            {
                // auto lm = m_NE->search(position, radius);
                close_landmarks.splice(close_landmarks.end(),m_NE->search(position, radius));
            }
            if(m_NW.get())
            {
                // auto lm = m_NW->search(position, radius);
    ...
    bool covers(const Position &pos, double radius)
    {      
        bool cover = false;
    ...
    bool overlapses(const Position &pos, double radius)
    {
        double dx = pos.x - m_center.x;
        double dy = pos.y - m_center.y;
        double d = sqrt(dx*dx + dy*dy);
...
private:
    ...
    {
        if (!tree.get()) // returns the pointer
        {
...
{
    srand(1231); // seed random number generator
    ...
    std::cout << "searching at x: " << x << " y: " << y << " radius: " << search_radius << std::endl;
    ...
    if (close_landmarks_quadtree.empty())
    {
